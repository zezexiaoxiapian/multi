...
from dataset import RECOVER_BBOXES_REGISTER, SAMPLE_GETTER_REGISTER, augment
...
def time_dataloader(config):
    meta_info = tools.build_model(
        config.model.cfg_path, device='cpu', dataparallel=False,
    )[0].meta_info()
    train_dataset = TrainDataset(config, meta_info)
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=config.train.batch_size,
        shuffle=False, num_workers=config.system.num_workers,
        pin_memory=True, collate_fn=collate_batch,
    )
    start = time.time()
    for _ in tqdm(train_dataloader):
        continue
    avg = (time.time() - start) / len(train_dataloader)
    print('{}s per batch'.format(avg))
    return avg
...
def evaluate(config, args):
    if args.onnx:
        ...
        model = torch_model_for_eval(args.cfg, args.weight, device=args.device, qat=args.qat, quant=args.quant)
        print(2)
...
def model_summary(_, args):
    ...
    radar = inputs[:, 4, :, :].reshape(inputs.shape[0], -1)
#    radar = tvm.nd.array(radar_numpy)
    flops, params = profile(model, inputs=(inputs, radar), verbose=False)
...
def time_forward(config, args):
    ...
    size = args.size
    avg_time = tools.compute_time(model, input_size=(3, size, size), batch_size=args.bs, device=args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--yaml', default='yamls/flir.yaml', required=False)
    parser.add_argument('--cfg',default='config', help='model cfg file', required=False)
    ...
    parser.add_argument('--bs', help='batch size', type=int, default=24)
